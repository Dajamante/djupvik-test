...
function getRelevantTimes(data) {
  ...
  /* update 21/08: also 16:00 */
  for (let i = 0; i < data.timeSeries.length; i++) {
    let current_time = new Date(data.timeSeries[i].validTime);
    if ([6, 12, 16, 18].includes(current_time.getHours())) {
...
function appendOrientedArrow(relevant_times) {
  for (let i = 0; i < 4; i++) {
    ...
    windSpeed.innerHTML =
      "(" + relevant_times[i].parameters[windSpeedIndex].values + ")";
...
function appendWeatherSymbol(relevant_times) {
  for (let i = 0; i < 4; i++) {
    ...
    let image_weather_values =
      relevant_times[i].parameters[image_index].values[0];
